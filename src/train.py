    ...
    if model_config["logistic_regression"]["enabled"]:
        specs.append(
            ModelSpec(
                ...
                param_grid=model_config["logistic_regression"]["param_grid"],
            )
        )
    ...
    if model_config["random_forest"]["enabled"]:
        specs.append(
            ModelSpec(
                ...
                param_grid=model_config["random_forest"]["param_grid"],
            )
        )

    if model_config["xgboost"]["enabled"]:
        scale_pos_weight = (
            _positive_class_weight(training_target)
        ...
        specs.append(
            ModelSpec(
                ...
                param_grid=model_config["xgboost"]["param_grid"],
            )
        )
    ...
    if model_config["catboost"]["enabled"]:
        specs.append(
            ModelSpec(
                ...
                param_grid=model_config["catboost"]["param_grid"],
            )
        )
...
def _comparison_table(results: list[ModelResult]) -> pd.DataFrame:
    ...
    for result in results:
        ...
        rows.append(row)
    return pd.DataFrame(rows).sort_values("validation_roc_auc", ascending=False)
